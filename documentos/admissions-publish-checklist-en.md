...
- Demo account uses safe sample data generated by `php artisan transport:seed-demo --reset`.
- Demo account is configured with `TRANSPORT_DEMO_READONLY=true`.
- `TRANSPORT_DEMO_EMAIL` matches `VITE_TRANSPORT_DEMO_EMAIL` so the login helper and API guard protect the same account.
- Demo navigation should show the real product modules while staying scoped to synthetic unit data.
