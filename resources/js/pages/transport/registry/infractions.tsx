...
export default function TransportRegistryInfractionsPage() {
    const [items, setItems] = useState<FineInfraction[]>([]);
    ...
    async function load(): Promise<void> {
        ...
        try {
            ...
            setNotification({
                message: 'Não foi possível carregar as infrações.',
                variant: 'error',
            });
        } finally {
            setLoading(false);
    ...
    function openEditDialog(item: FineInfraction): void {
        setEditingItem(item);
        setFormData({
            nome: item.nome,
            ativo: item.ativo,
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>): Promise<void> {
        ...
        try {
            if (editingItem) {
                await apiPut(`/registry/infracoes-multa/${editingItem.id}`, {
                    nome: formData.nome.trim(),
                    ativo: formData.ativo,
                });
            } else {
                await apiPost('/registry/infracoes-multa', {
                    nome: formData.nome.trim(),
                    ativo: formData.ativo,
                });
            }
            ...
            if (error instanceof ApiError) {
                ...
                setNotification({
                    message: 'Não foi possível salvar a infração.',
                    variant: 'error',
                });
            }
        } finally {
    ...
    async function confirmDelete(): Promise<void> {
        ...
        try {
            await apiDelete(`/registry/infracoes-multa/${deleteCandidate.id}`);
            setNotification({
            ...
            if (error instanceof ApiError) {
                ...
                setNotification({
                    message: 'Não foi possível excluir a infração.',
                    variant: 'error',
                });
            }
        } finally {
    ...
    return (
        <AdminLayout title="Cadastro - Infrações" active="registry-infractions" module="registry">
            <div className="space-y-6">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Infrações ({items.length})</CardTitle>
                    </CardHeader>
                    <CardContent>
                        {loading ? (
