...
function compressRows(
    ...
    const other: ChartRow = {
        label: 'Outros',
        quantidade: tail.reduce(
            (sum, item) => sum + Number(item.quantidade ?? 0),
            0,
        ),
        valor_total: tail.reduce(
...
const ColumnComparisonCard = memo(function ColumnComparisonCard({
    ...
    const compactRows = useMemo(
        () => compressRows(rows, 'valor_total', 7),
        [rows],
    );
                        ...
                        <div className="overflow-x-auto pb-1">
                            <div className="flex min-w-[560px] items-end gap-4">
                                {compactRows.map((row, index) => {
                                    const qtyPercent =
                                        maxQty > 0
                                            ? (row.quantidade / maxQty) * 100
                                            : 0;
                                    const valuePercent =
                                        maxValue > 0
                                            ? (row.valor_total / maxValue) * 100
                                            : 0;
                                    const qtyHeight = Math.max(
                                        8,
                                        Math.round((qtyPercent / 100) * 180),
                                    );
                                    const valueHeight = Math.max(
                                        8,
                                        Math.round((valuePercent / 100) * 180),
                                    );
...
export default function TransportFinesDashboardPage() {
        ...
        async (
            ...
            try {
                ...
                const response = await apiGet<DashboardResponse>(
                    `/fines/dashboard?${params.toString()}`,
                );
                ...
                setData({
                    ...
                    charts: {
                        ...response.charts,
                        culpa: response.charts.culpa.map((row) => ({
                            ...row,
                            label: culpaLabel(row.label),
                        })),
                        tipo_valor: response.charts.tipo_valor.map((row) => ({
                            ...row,
                            label: tipoValorLabel(row.label),
                        })),
                        status: response.charts.status.map((row) => ({
                            ...row,
    ...
    return (
        <AdminLayout
            title="Gestão de Multas - Dashboard"
            active="fines-dashboard"
            module="fines"
        >
            <div className="transport-dashboard-page">
                <div className="transport-dashboard-header">
                    ...
                    <p className="transport-dashboard-subtitle">
                        Acompanhe quantidade e valor de multas por infração,
                        culpa, status, placa e motorista.
                    </p>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <Card className="transport-metric-card transport-tone-warning">
                        ...
                        <CardContent>
                            <p className="transport-metric-value">
                                {formatIntegerBR(
                                    data?.totals.vencidas_em_aberto
                                        .quantidade ?? 0,
                                )}
                            </p>
                            <p className="transport-metric-context">
                                {formatCurrencyBR(
                                    data?.totals.vencidas_em_aberto.valor ?? 0,
                                )}{' '}
                                em risco.
                            </p>
                        </CardContent>
                ...
                {loading && !data ? (
                    ...
                    <>
                                ...
                                <CardContent className="space-y-3 text-sm">
                                    <div className="transport-list-panel">
                                        ...
                                        <p className="mt-1 font-semibold">
                                            {data?.totals.top_placa?.label ??
                                                '-'}
                                        </p>
                                        <p className="text-xs text-muted-foreground">
                                            {formatCurrencyBR(
                                                data?.totals.top_placa
                                                    ?.valor_total ?? 0,
                                            )}
                                        </p>
                                    </div>
                        ...
                        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
                            <LazyChartSlot>
                                <PieSummaryCard
                                    title="Gráfico por Infração (Quantidade)"
                                    rows={data?.charts.infracao ?? []}
                                    mode="quantidade"
                                    colorSeed={0}
                                />
                            </LazyChartSlot>
                            <LazyChartSlot>
                                <PieSummaryCard
                                    title="Gráfico por Infração (Valor)"
                                    rows={data?.charts.infracao ?? []}
                                    mode="valor_total"
                                    colorSeed={1}
                                />
                            ...
                            <LazyChartSlot>
                                <PieSummaryCard
                                    title="Gráfico por Culpa (Quantidade)"
                                    rows={data?.charts.culpa ?? []}
                                    mode="quantidade"
                                    colorSeed={2}
                                />
                            </LazyChartSlot>
                            ...
                            <LazyChartSlot>
                                <PieSummaryCard
                                    title="Gráfico por Tipo Valor"
                                    rows={data?.charts.tipo_valor ?? []}
                                    mode="quantidade"
                                    colorSeed={4}
                                />
                            </LazyChartSlot>
                            <LazyChartSlot>
                                <PieSummaryCard
                                    title="Gráfico por Status"
                                    rows={data?.charts.status ?? []}
                                    mode="quantidade"
                                    colorSeed={5}
                                />
                            </LazyChartSlot>
                        ...
                        <div className="grid gap-4 xl:grid-cols-2">
                            <LazyChartSlot minHeight={360}>
                                <ColumnComparisonCard
                                    title="Gráfico de Colunas por Placa (qtde e valor)"
                                    rows={data?.charts.placa ?? []}
                                />
                            </LazyChartSlot>
                            <LazyChartSlot minHeight={360}>
                                <ColumnComparisonCard
                                    title="Gráfico de Colunas por Motorista (qtde e valor)"
                                    rows={data?.charts.motorista ?? []}
                                />
                            </LazyChartSlot>
                        </div>
