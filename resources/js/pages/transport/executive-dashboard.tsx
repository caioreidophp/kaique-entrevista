...
function priorityLabel(
    priority: PayrollApprovalsResponse['data'][number]['priority'],
): string {
...
function approvalActionLabel(actionKey: string): string {
    if (actionKey === 'payroll.launch-batch')
        return 'Folha - lancamento em lote';
    if (actionKey === 'fines.entry.store') return 'Multas - novo lancamento';
    if (actionKey === 'fines.entry.update') return 'Multas - alteracao';
    if (actionKey === 'vacations.entry.store')
        return 'Ferias - novo lancamento';
...
export default function TransportExecutiveDashboardPage() {
    ...
    const [approvalActionById, setApprovalActionById] = useState<
        Record<number, 'approve' | 'reject' | null>
    >({});
    const [state, setState] = useState<ExecutiveDashboardState | null>(null);

    async function refreshApprovals(): Promise<void> {
        const approvals = await apiGet<PayrollApprovalsResponse>(
            '/payroll/approvals?status=pending&per_page=5',
        );
    ...
    async function handleApprovalAction(
        ...
        try {
            ...
            if (requestError instanceof ApiError) {
                ...
                setNotification({
                    message:
                        'Nao foi possivel concluir a revisao da solicitacao.',
                    variant: 'error',
                });
            }
        } finally {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            apiGet<PayrollApprovalsResponse>(
                '/payroll/approvals?status=pending&per_page=5',
            ),
        ])
            .then(
    ...
    const mergedAlerts = useMemo(() => {
        if (!state) return [];
        return [...state.executive.alerts, ...state.observability.alerts].slice(
            0,
            8,
        );
    }, [state]);
    ...
    const maxPendingPressure = Math.max(
        1,
        ...topPendingUnits.map((item) => item.pressure_score),
    );
    const maxQualityIssues = Math.max(
        1,
        ...worstQualityUnits.map((item) => item.total_issues),
    );
            ...
            <div className="transport-dashboard-page">
                <div className="transport-dashboard-header">
                    ...
                    <p className="transport-dashboard-subtitle">
                        Consolidado de operacao, pendencias, qualidade de dados
                        e saude do sistema.
                    </p>
                </div>
                    ...
                    <>
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            <InfoMetric
                                ...
                                value={formatPercentBR(
                                    state.executive.interviews.approval_rate,
                                )}
                            ...
                            <InfoMetric
                                ...
                                value={formatPercentBR(
                                    state.executive.payroll.coverage_rate,
                                )}
                            ...
                            <InfoMetric
                                ...
                                value={formatIntegerBR(
                                    state.approvals.summary.pending,
                                )}
                        ...
                        <div className="grid gap-4 xl:grid-cols-[1.15fr_1fr]">
                            <Card className="transport-insight-card">
                                <CardHeader>
                                    ...
                                    <p className="transport-dashboard-section-subtitle">
                                        Comparativo de volume e custo
                                        operacional consolidado no mes atual.
                                    </p>
                                </CardHeader>
                                    ...
                                    <div className="space-y-2">
                                        {benchmarkTopUnits.length === 0 ? (
                                            <div className="transport-empty-state">
                                                <strong>
                                                    Sem benchmark no periodo
                                                </strong>
                                                Nao ha dados suficientes para
                                                comparar unidades neste mes.
                                            </div>
                                        ) : (
                                            benchmarkTopUnits.map(
                                                (unit, index) => (
                                                    <div
                                                        key={unit.unidade_id}
                                                    ...
                                                    >
                                                        <div className="flex items-center justify-between gap-2">
                                                            <div className="flex min-w-0 items-center gap-2">
                                                                <span className="transport-value-pill shrink-0">
                                                                    #{index + 1}
                                                                </span>
                                                            ...
                                                            <span className="text-xs text-muted-foreground">
                                                                {formatPercentBR(
                                                                    unit.operation_cost_share_percent,
                                                                )}{' '}
                                                                do custo
                                                            </span>
                                                        </div>
                            ...
                            <Card className="transport-insight-card">
                                <CardHeader>
                                    ...
                                    <p className="transport-dashboard-section-subtitle">
                                        Projecao de ferias e previsao de folha
                                        para planejamento operacional.
                                    </p>
                                </CardHeader>
                                <CardContent className="space-y-3">
                                    <div className="grid gap-3 sm:grid-cols-3">
                                    ...
                                    <div className="transport-list-panel">
                                        <p className="text-xs text-muted-foreground">
                                            Previsao de folha (media ultimos 3
                                            meses)
                                        </p>
                                        <p className="mt-1 text-lg font-semibold">
                                            {formatCurrencyBR(
                                                state.forecast.payroll_forecast
                        ...
                        <div className="grid gap-4 xl:grid-cols-[1.45fr_1fr]">
                                        ...
                                        mergedAlerts.map((alert, index) => (
                                            ...
                                            >
                                                <div className="flex items-start gap-2">
                                                    <AlertTriangle
                                                        className={`mt-0.5 size-4 ${alertIconClass(alert.level)}`}
                                                    />
                                                    <div>
                            ...
                            <Card className="transport-insight-card">
                                ...
                                <CardContent className="space-y-3 text-sm">
                                    <div className="transport-list-panel">
                                        <p className="text-xs text-muted-foreground">
                                            Latencia mais lenta (p95)
                                        </p>
                                        <p className="mt-1 text-xl font-semibold">
                                            {formatIntegerBR(
                                                Math.round(
                                                    state.observability.latency
                                                        .slowest_p95_ms ?? 0,
                                                ),
                                            )}{' '}
                                            ms
                        ...
                        <div className="grid gap-4 xl:grid-cols-[1.2fr_1fr_1fr]">
                            <Card className="transport-insight-card">
                                <CardHeader>
                                    ...
                                    <p className="transport-dashboard-section-subtitle">
                                        Priorização por volume e urgência
                                        operacional.
                                    </p>
                                </CardHeader>
                                <CardContent className="space-y-2">
                                    {topPendingUnits.length === 0 ? (
                                        <div className="transport-empty-state">
                                            <strong>
                                                Sem pendencias consolidadas
                                            </strong>
                                            Nao ha pendencias agrupadas por
                                            unidade para o periodo.
                                        </div>
                                    ) : (
                                            ...
                                            >
                                                ...
                                                <div className="transport-progress-track mt-2">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${Math.min(100, Math.max(8, (unit.pressure_score / maxPendingPressure) * 100))}%`,
                                                        }}
                                                    />
                                                </div>
                                                <div className="mt-2 grid gap-2 sm:grid-cols-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="font-semibold">
                                                            {formatIntegerBR(
                                                                unit.onboarding_overdue,
                                                            )}
                                                        </p>
                                                    </div>
                                                    <div>
                                                        <p className="text-[11px] text-muted-foreground">
                                                            Cancelados a receber
                                                        </p>
                                                        <p className="font-semibold">
                                                            {formatIntegerBR(
                                                                unit.freight_canceled_to_receive,
                            ...
                            <Card className="transport-insight-card">
                                <CardHeader>
                                    ...
                                    <p className="transport-dashboard-section-subtitle">
                                        Campos faltantes e distribuicao de gaps
                                        por unidade.
                                    </p>
                                </CardHeader>
                                <CardContent className="space-y-3 text-sm">
                                    ...
                                    {worstQualityUnits.length > 0 ? (
                                        <div className="space-y-2">
                                            <p className="text-xs font-medium text-muted-foreground">
                                                Unidades com mais gaps
                                            </p>
                                            {worstQualityUnits.map(
                                                (row, index) => (
                                                    <div
                                                        key={row.unidade_id}
                                                        className="transport-list-panel"
                                                    >
                                                        <div className="flex items-center justify-between gap-2">
                                                            <div className="flex items-center gap-2">
                                                                <span className="transport-value-pill">
                                                                    #{index + 1}
                                                                </span>
                                                                <span>
                                                                    Unidade #
                                                        ...
                                                        <div className="transport-progress-track mt-2">
                                                            <div
                                                                ...
                                                                style={{
                                                                    width: `${Math.min(100, Math.max(8, (row.total_issues / maxQualityIssues) * 100))}%`,
                                                                }}
                                                            />
                                                        </div>
                                                    </div>
                                ...
                                <CardContent className="space-y-2">
                                    {state.approvals.data.length === 0 ? (
                                        <div className="transport-empty-state">
                                            ...
                                            Nao ha solicitacoes financeiras
                                        ...
                                        state.approvals.data.map((approval) => (
                                            ...
                                            >
                                                <div className="flex items-center justify-between gap-2">
                                                    ...
                                                    <span
                                                        className={`transport-status-badge ${priorityToneClass(approval.priority)}`}
                                                    >
                                                        {priorityLabel(
                                                            approval.priority,
                                                        )}
